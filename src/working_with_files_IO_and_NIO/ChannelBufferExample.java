...
контейнер для извлечения, к примеру, сахара из миски, а также как небольшой контейнер для добавления сахара в миску
извне. Таким образом, ложка действует как Buffer, а миска - как Channel.
Да, при использовании стримов можно было обернуть в Buffered Stream (поток) в Java IO, но все же самую важную роль в этой
концепции играет сам стрим, а не его буферизация.
...
Запись в файл: Channel читает информацию из Buffer и записывает ее в файл. Естественно, эту информацию необходимо записать
в Buffer.

2. Channel может читать и записывать асинхронно.

3. Channel и Buffer имеют связь друг с другом - Channel всегда работает с Buffer. Как было ранее упомянуто, Buffer - это блок
памяти, в который можно записывать информацию и читать информацию из этого блока памяти. Используя Buffer возможно читать
информацию из него, а затем возвратиться к уже прочитанной информации и снова ее просматривать, т.е. "гулять" по буферу
туда-сюда, чего невозможно сделать в Stream (поток) в Java IO. Просто нужно манипулировать с Buffer для обработки данных,
считанных с Channel, и манипулировать с Buffer для записи данных в Channel.
...
Теперь можно перейти к чтению из файла. Для начала нужно создать буфер, буфер будет заведомо меньшего размера, чем размер
самого стиха, чтобы понять принцип работы с буфером:

        ByteBuffer buffer = ByteBuffer.allocate(25);

Этой записью создается буфер с вместимостью в 25 байт. Если попытаться после полного заполнения буфера добавить в буфер еще,
хотя бы, 1 байт, то будет выброшено исключение, сигнализирующее о том, что буфер уже полный и места для новых байт нет. Т.е.
данный буфер может быть заполнен, максимум, на 25 байт.
...
Метод read ВОЗВРАЩАЕТ КОЛИЧЕСТВО прочитанных байт и запоминает в переменную типа int - byteRead. Т.е. читается информация с
помощью channel из файла и сразу записывается в буфер ( channel.read(куда записываем прочитанную информацию?) ), а КОЛИЧЕСТВО
прочитанных байт запоминается в переменной byteRead.
Естественно, за один раз (когда этот код channel.read(buffer); сработает один раз) весь текст стиха не уместиться в буфер
размером в 25 байт, поэтому нужно использовать цикл while до тех про, пока есть, что читать из файла:

        while (byteRead > 0) {

Пока переменная byteRead, в которой запоминается КОЛИЧЕСТВО прочтенных байт, будет больше нуля - это будет означать, что из
файла есть, что читать.
В цикле будет выводится каждый раз количество прочитанных байтов (для наглядности работы буфера):

        System.out.println("How many bytes were read from the file and written to the buffer?: " + byteRead + " bytes;");
...
Буфер, как ранее упоминалось, - это блок в памяти, в который можно что-то записать и который потом можно использовать для
...
Понятия Position и Limit зависят от того, происходит запись в буфер или происходит чтение из буфера. Capacity означает всегда
одно и тоже - вместимость буфера, определенный его размер. Если речь идет о ByteBuffer, то возможно записать в буфер количество
байтов не превышающее его вместимость, т.е. не превышающее его capacity. В данном случае capacity равен 10 байт. Представим,
что файл, с которым происходит работа, содержит 23 байта и был создан буфер вместимостью всего 10 байт.
Для начала рассмотрим WRITE MODE.
Буфер содержит 10 мест для записи байтов, будем считать, что сверху отчет начинается от 0 до 9. Capacity буфера - 10 элементов.
Когда что-то записывается в буфер, то это запись делается на определенной позицию - Position. Изначально позиция равна нулю. Когда
что-то записывается в буфер, в данном случае, записываются байты, то позиция меняется. Записали какой-то байт:
...
Теперь позиция в буфере указывает на 1. При следующей записи, байт уже записывается на позицию 1 и теперь позиция в буфере
указывает на 2.
...
После заполнения буфера Position находится за пределами области памяти, которая была выделена для буфера, т.е. на позиции
10, куда больше ничего записать уже нельзя, иначе выбросится исключение. Теперь нужно читать из буфера. НО, а как быть,
если Position находится на позиции 10? В этом поможет метод flip. Метод flip меняет режим буфера, т.е. переводит буфер
из режима записи в режим чтения. Когда вызывается метод flip, Position меняется с 10 из WRITING MODE в позицию 0 для
READING MODE:
...
Чтение начинается с позиции 0, затем с позиции 1, затем с позиции 2 и тд. до позиции, когда чтение происходит с позиции 9.
После чего вызывается другой метод - метод clear. Когда вызывается метод clear (с англ. чистый), то на самом деле данные
в буфере не очищаются, т.е. метод clear не удаляет информацию из буфера, а просто Position возвращается на нулевой
элемент и меняет режим READ на режим WRITING. Когда происходит очередная запись информацию в буфер, то в каждой ячейки
старый байт затирается и записывается в эту ячейку новый байт, т.е. происходит перезапись информации в каждой ячейки
буфера. Метод clear не удаляет информацию из буфера, но при записи новой старая информация затирается, новые байты
записываются поверх старых.

Допустим, так случилось, что Position был на 4 элементе в режиме READING, а в этот момент был вызван метод clear. Что
произойдёт?
...
количество байтов, которое возможно прочитать. В этом примере файл содержит 23 байта, емкость буфера 10 байт.
Записались первые 10 байтов в буфер. Вызывается метод flip и из буфера читаются все 10 байт, 10 байт потому, что
последняя позиция в режиме записи была на 9 элементе (отчет с нуля и capacity - 1). В режиме READ устанавливается
Limit на позиции 9, тем самым сообщая, что можно прочитать весь буфер.
...
установлена позиция в режиме WRITE на 0 ячейку. Два раза по 10 байт уже было записано, осталось записать 3 байта из 23-х.
Записываются оставшиеся 3 байта в буфер:
...
Тут конечная позиция на ячейки 3 и когда вызывается метод flip, то в режиме READ меняется Position на ячейку 0, но
...
затем из ячейки 2 и достигается Limit . Благодаря Limit мы понимаем, что дальше Limit читать не нужно, т.к. буфер
был заполнен до Limit-a, до ячейки 2.
 */
public class ChannelBufferExample {
    public static void main(String[] args) {
        try (RandomAccessFile file = new RandomAccessFile("forChannelBufferExample.txt", "rw");
            ...
            /*
            Если попытаться после его полного заполнения добавить в буфер еще хотя бы один байт, то будет выброшено
...
How many bytes were read from the file and written to the buffer?: 25 bytes;
How many bytes were read from the file and written to the buffer?: 25 bytes;
How many bytes were read from the file and written to the buffer?: 6 bytes;
