...
должен выполнять новый поток. К примеру, класс MyThread1 будет наследовать (extends) класс Thread и
в методе run будет заложена логика вывода чисел на экран от 1 до 1_000. Второй класс MyThread2 тоже
наследует класс Thread и в его методе run заложена логика вывода чисел на экран от 1_000 до 1 (в
...
В методе main создаются два объекта MyThread1 и MyThread2.
...
Если не переопределять (override) метод run, к примеру, не сделать этого в классах MyThread1, MyThread2
и запустить этот код, то ничего не произойдет. Метод run в классе Thread ничего не делает, он пустой:
...
Так же стоит обратить внимание на то, что в классах MyThread1 и MyThread2 переопределяется метод run,
а запускаются потоки при помощи метода start. Метод run вызывается автоматически JVM после вызова
метода start.
...
необходимо указать, что необходимо делать потоку.
        ...
        class MyThreadImplementsRunnable_1 implements Runnable {
            @Override
            public void run() {
                for (int i = 1; i <= 1_000; i++) {
                    System.out.println(i);
        ...
        class MyThreadImplementsRunnable_2 implements Runnable {
            @Override
            public void run() {
                for (int i = 1_000; i >= 1; i--) {
                    System.out.println(i);
...
интерфейс Runnable:
        ...
        Thread thread1 = new Thread(new MyThreadImplementsRunnable_1());
        Thread thread2 = new Thread(new MyThreadImplementsRunnable_2());
...
Вне зависимости от того, какой способ будет использован, обязательно необходимо переопределять метод run и
...
тоже поток. Вот этот поток отпачковуется от main потока и работает обособленно:

        VariantsCreatingThreading thread3 = new VariantsCreatingThreading();
        thread3.start();

а в методе main можно указать само тело кода, который был бы в методе run:
...
имплементирует функциональный интерфейс Runnable. Можно использовать анонимный класс:
...
Еще быстрее можно создать используя lambda выражение:
