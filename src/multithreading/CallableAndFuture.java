...
public class CallableAndFuture {
    ...
    public static void main(String[] args) throws InterruptedException {
        ExecutorService executorService = Executors.newSingleThreadExecutor(); // создается Thread pool с одним потоком
        Factorial factorial = new Factorial(5);
        executorService.execute(factorial); // метод execute передает задание в Thread pool
        executorService.shutdown(); // метод shutdown заканчивает работу Thread pool
        executorService.awaitTermination(10, TimeUnit.SECONDS); // метод awaitTermination просит подождать поток main
...
Приходится использовать переменную factorialResult вне класса, чтобы передать значение result. Потому, что тип
возвращаемого значения метода run - void, он ничего не возвращает. Так же невозможно в методе run выбросить какое-либо
исключение. К примеру, если в примере выше кто-то введет неверное число вместо System.out.println("Вы ввели не верное
число"); выбросить исключение:
        public void run() {
        if (f <= 0) {
            System.out.println("Вы ввели не верное число");
            return; // заканчивается метод, если число меньше или равно 0
...
называется метод call (у Runnable - run, Callable - call). Этот метод может выбрасывать Exception и имеет return type -
(V). Тут используются Generics, т.е. когда происходит создание класса имплементирующего функциональный интерфейс
Callable, нужно указать с каким типом будет этот класс работать. Т.е. какой тип данных будет в output-e у метода call:

        class Factorial implements Callable<Integer> {  <- тут указывается тип данных (Integer)
        ...
            @Override
            public Integer call() throws Exception { <- тут указывается тип данных (Integer)
                ...
            }
        }
...
public class CallableAndFuture {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        потому, что метод call это допускает:
                ...
                @Override
                public Integer call() throws Exception { <- допускает выбросить исключение метод call
                    if (f <= 0) {
                        throw new Exception("Вы ввели не верное число"); <- тут то выбрасывается исключение
                    }
                    int result = 1;
                    for (int i = 1; i <= f; i++) {
                        result *= i;
        ...
        Поэтому, когда происходит попытка получить результат задания при помощи метода get, может выброситься исключение
        и поэтому необходимо учесть этот момент. Выброситься может исключение ExecutionException и если это исключение
        будет поймано, то при помощи метода getCause (с англ. получить причину) можно узнать причину по которой был
        выброшен Exception. А что эта причина покажет вот это сообщение: "Вы ввели не верное число".
         */
        try {
        ...
        } finally { //ExecutorService всегда хорошо закрывать в finally блоках
