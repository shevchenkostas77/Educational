...
объектах, и эти потоки используют синхронизацию на этих объектах не в одинаковом
порядке.
*/
...
Монитор объекта lock2 захвачен". Далее он пытается захватить монитор объекта lock1
...
/*
Livelock - ситуация, когда два или более потоков залочены навсегда, ожидают друг
друга, проделывают какую-то работу, но без какого-либо прогресса.
Представим, что есть мост дугообразной формы и этот мост с автомобильной дорогой.
Есть две машины, одна машина с одной стороны моста и вторая машина с другой стороны
моста. Эти машины - машины-роботы, т.е. управление этих машин происходит при помощи
искусственного интеллекта. На этом мосту автомобильная дорога для очень узкая и по
ней может проехать только одна машина. Машины по бокам моста заранее не видят друг
друга. Суть работы машины робота заключается в том, когда эта машина въезжает на
мост и видит, что едет на встречу другая машина, то машина-робот должна уступить
дорогу, т.е. вернуться назад (в начало моста), подождать, чтобы другая машина
проехала и только после этого продолжить свое движение. Представим, что две
робот-машины (с обеих сторон моста) начали одновременно забираться на этот мост.
Что произойдет тогда? В какой-то момент времени эти роботы-машины увидят друг друга
и у каждой из них сработает команда, что нужно уступить дорогу. Обе машины
возвращаются в исходное положение. Ждут какое-то время, видят, что другая машина
не проехала и снова поднимаются на мост. И процесс повторяется снова. Это называется
Livelock и будет работать бесконечно, как и DeadLock, но совершается какае-то работа,
прогресс которой равен нулю. Еще один пример. Есть потоки T1 и Т2. И есть какой-то
ресурс. Поток Т1 делает какие-то изменения в этом ресурсе, а поток Т2 эти изменения
постоянно стирает. Т.е. потоки Т1 и Т2 вроде бы работают, но конечного прогресса
у них нет. Они будут так работать "вечно". Отличие от Deadlock в том, что при
Deadlock-е потоки ничего не делают, просто ждут, а в LiveLock-e проделывают какую-то
работу, но напрасно.
*/

/*
Lock starvation (starvation с англ. голодание) - ситуация, когда менее приоритетные
потоки ждут долгое время или все время для того, чтобы могли запуститься.
Представим, что у нас множество потоков, допустим это: Т1, Т2, Т3, Т4, Т5. И эти
потоки проделывают какую-то работу. Но метод, в котором потоки проделывают свою
работу, синхронизирован так, что только один поток в одно и то же время может зайти
в метод и проделать какую-то работу. Потоки Т1, Т2 и Т3 имеют высокий приоритет.
Поток Т1 заходит в метод и начинает проделывать какую-то работу длительное время,
а остальные потоки ждут. После того как поток Т1 закончил свою работу, поток Т2
начинает свою. В это же время создается еще один поток, к примеру, это поток Т6
и он с высоким приоритетом. Поток Т2 заканчивает свою работу и начинает свою работу
поток Т3 и у него тоже высокий приоритет. Во время работы потока Т3 создается еще
какой-то поток, к примеру, это будет поток Т7 и тоже с высоким приоритетом. После
того, как поток Т3 закончил работу, lock объекта на котором идет синхронизация
захватывает поток Т6, потом захватит Т7, потом захватят новосозданные потоки с
высоким приоритетом. И выходит, что потоки с более низким приоритетом, а в данном
случае, это потоки Т4 и Т5, непонятно, будут вообще работать или нет. У них низкий
приоритет. Остальные потоки долго работают, во время их работы создаются новые
потоки с высоким приоритетом и получается, что потоки Т4 и Т5 голодают.
*/

// НУЖНО стараться не допускать этих ситуаций!